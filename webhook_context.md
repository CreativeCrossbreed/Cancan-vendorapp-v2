# WhatsApp Webhook Context

Here is the codebase context related to the WhatsApp Webhook integration for the vendor app. This information should be comprehensive enough to allow Claude to understand the current implementation and plan updates.

---

## 1. Webhook Handler (`frontend/src/app/api/whatsapp/webhook/route.ts`)

This is the main endpoint that receives incoming messages and events from Meta's WhatsApp Business API, verifies the webhook signatures, and processes them through a state machine for customer onboarding and ordering.
...
## 2. Configuration API (`frontend/src/app/api/whatsapp/config/route.ts`)

This is the internal endpoint used by the admin dashboard's frontend to fetch WhatsApp configuration credentials stored securely in the Supabase database.
...
## 3. Webhook Configuration in the Admin Portal

### **Settings Page (`frontend/src/app/portal/(dashboard)/settings/page.tsx`)**
This React component manages the backend configuration variables used by the Webhook code:
- **`whatsapp_api_token`**: Stored in DB table `app_settings`.
- **`whatsapp_phone_number_id`**: Stored in DB table `app_settings`.
- **`whatsapp_webhook_secret`**: Stored in DB table `app_settings`. Used by `GET /api/whatsapp/webhook` to verify the webhook setup handshake.
- **`whatsapp_business_account_id`**: Stored in DB table `app_settings`.
- **`meta_app_secret`**: Stored in DB table `app_settings`. Used by `POST /api/whatsapp/webhook` to verify the HMAC signature of incoming POST requests.

### **WhatsApp Admin Panel (`frontend/src/app/portal/(dashboard)/whatsapp/page.tsx`)**
This page allows admins to:
- Monitor **API Status** (Connected/Disconnected based on the configuration above).
- Inspect **Total Messages** and read inbound & outbound webhooks stored in the `whatsapp_messages` table via `fetchWhatsAppMessages`.
- View **WhatsApp Orders** received from the webhook interaction, fetched from the `whatsapp_orders` table (wait, currently orders are partially mocked/missing from the endpoint, see the TODO: "Ideally, here we insert into the orders table").

## Summary for Refactoring / Planning
When you provide this file to Claude, note that:
- Authentication & Handshake (`GET`) is fully implemented with environment/DB variable checks.
- Event listening (`POST`) has signature verification and a basic Onboarding State Machine utilizing the `whatsapp_sessions` database table.
- Order extraction (`confirm_order` action) currently just sends a placeholder response instead of actually persisting into an `orders` table.
