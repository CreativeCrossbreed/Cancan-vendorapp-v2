...
class VendorDataService {
  ...
  static Future<Map<String, dynamic>?> getVendorProfile({
    ...
    if (!forceRefresh) {
      ...
      if (cached != null) {
        ...
        debugPrint('📦 Loaded vendor data from local cache');
        return cached;
      }
    }

    // Fetch from Supabase
    debugPrint('🌐 Fetching vendor data from Supabase...');
    try {
      final vendorId = SupabaseConfig.currentVendorId;
      if (vendorId == null) {
        debugPrint('⚠️ No vendor ID found');
        return null;
      }
      ...
      if (data != null) {
        ...
        debugPrint('✅ Vendor data cached: ${data['name']}');
      }
  ...
  static Future<void> _saveToPrefs(Map<String, dynamic> data) async {
    try {
      ...
      debugPrint('❌ Error saving vendor data to prefs: $e');
    }
  }
  ...
  static Future<Map<String, dynamic>?> _loadFromPrefs() async {
    try {
      ...
      debugPrint('❌ Error loading vendor data from prefs: $e');
    }
    return null;
  }
  ...
  static Future<void> clearCache() async {
    ...
    try {
      ...
      debugPrint('🗑️ Vendor data cache cleared');
    } catch (e) {
  ...
  static Future<void> updateCache(Map<String, dynamic> newData) async {
    ...
    debugPrint('✅ Vendor data cache updated');
  }
  ...
  static Future<void> initialize({bool forceRefresh = false}) async {
    debugPrint('🚀 Initializing VendorDataService...');
    await getVendorProfile(forceRefresh: forceRefresh);
  }

  /// Update vendor profile and refresh cache
