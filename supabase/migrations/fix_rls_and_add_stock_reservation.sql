...
-- Allow vendors to select products
...
-- Allow vendors to update products
...
-- Step 4: Fix policies for orders table (if not already fixed)
...
BEGIN
  ...
  SET reserved_stock = vp.reserved_stock - oi.quantity
  FROM order_items oi
...
BEGIN
  UPDATE vendor_products vp
  SET current_stock = vp.current_stock - oi.quantity,
      reserved_stock = vp.reserved_stock - oi.quantity
  FROM order_items oi
  JOIN orders o ON oi.order_id = o.id
  WHERE o.id = p_order_id
    AND vp.vendor_id = o.vendor_id
